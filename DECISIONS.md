# 实施决策（v5 高容量版，P0–P17 synthetic/tiny CPU 工程门禁与 P18 runtime 骨架门禁已通过）
...
SGD、full-second-order meta 路与模块 gradient/delta 审计的合成工程门禁；P15 已以
synthetic/tiny A2 验证 typed target、hard/soft 状态路径、Retriever/Reader/Qwen prefill、
State+Answer Outer step、指标、trainable-only checkpoint 与 fail-closed exit gate；P16 已以 A3
验证单 Support→functional SGD→后续 Query→outer gradient 的 Stage B 闭环；P17 已以 A4/A5
验证 identity/event consistency、1/4/8 Support、多 Query 与 bounded CPU graph 回收的 Stage C
工程闭环，但独立 missed-new runner 指标和 GPU 显存证据仍待补；P18 仅验证逐视频 reset/release、
因果 chunk、Fast Adapter state 绑定、update stage 注入契约、单次 prefill 与 immutable decode 的
runtime 骨架。生产 `TTTUpdateStage` 尚未接入真实 `L_TTT`→functional SGD，真实 Qwen3-VL-8B
`GenerationDriver` 也尚未运行；训练、生成、性能和科学效果仍属于 P19–P22。详细论证见
...
5. P3 `qwen_adapter.py`、P4 `query_encoder.py`、P5 `fast_ttt.py` 及 P6–P17 对应模块已通过各自
   synthetic/tiny CPU 工程门禁；P18 仅通过 runtime 骨架门禁。P19 及后续模块仍按阶段 fail closed；
   模块可导入不等于后续算法已实现。
...
5. 显式传入 per-row `fast_state` 是 P14/测试的 functional 边界：online state 使用 detached leaf
   ...
   参数集合。
6. P18 runtime 骨架使用 module-local `use_fast_state()` 受管生命周期兼容 P3 调用：拒绝 stale
   module grad，进入时冻结 checkpointed 参数的 `requires_grad`，退出时恢复，并保证
   exception-safe、非重入。video ID/batch order 对齐、并发串行化或实例隔离，以及受管调用必须
   产生 `last_audit.used_runtime_state=True` 的系统级证明已由 P18 验证，不倒推为 P5 当时的验收范围。
...
8. P5 只冻结 Adapter、state/reset、参数分组和数值边界；实际 loss、gradient clip 和一步 SGD 由
   P14 实现，P18 只实现受管推理 runtime 骨架。P5 本地验收不得被表述为真实 8B、真实视频、
...
9. P6 只拥有空间路和可复用的无状态 grid helper；P7 拥有时间池化/因果 Transformer/cache，
   P8/P9 拥有对象语义与 hard state，P13 拥有模型编排，P18 拥有受管 runtime 骨架；生产更新与
   真实 8B generation 仍留 P19+。
...
2. O2/E1/E2 overlap 全部以 current prediction 为 source、detached previous snapshot 为 target；
   ...
   low-confidence/invalid-source/padding 均强类型审计；跨 chunk snapshot/match 生命周期已由 P17
   编排并验证。
...
7. online 更新只产生独立 leaf 的下一代 `W_t`；meta 固定 `meta_full_second_order`，
   ...
   result audit 与测试；P16 已提供显式 first-order 解析参考，真实实验比较仍留 P21。
...
10. P14 所有证据均来自进程内合成张量和小型模块链；未下载视频、数据集或 8B 权重，不支持
    训练收敛、真实精度或 TTT 科学增益结论。O1 target builder 当时留 P15，现已按
    独立 typed-label 边界实现；真实模型/资产仍留 P19。

## P15 已验证边界

1. 低空间 Stage A 固定为 A2、`static_w0_no_inner_sgd` 和
   `frozen_synthetic_engineering_gate`；Qwen allowlist 为空。static `W0`、Query、时空
   Encoder、四 Head、Semantic Projector/Bank component 与 Resampler 按显式 allowlist 进入
   Outer AdamW；Predictor、functional SGD、transient `W_t`、hard runtime 和 Qwen 全部排除。
2. State target builder 只接收独立标签，并固定
   `official_explicit`/`synthetic_explicit`/`missing` provenance。O1 使用 pre-matched 六字段
   slot/mask，O2/E1/E2、operator、time 和 record relevance 按 typed row 对齐；missing 不得
   变成零标签，也不得从最终 count 反推 dense assignment。Answer 标签独立保存，
   Reader number 只是 context，不得变成 teacher-forced supervision。
3. A2 episode 按 owner reset，因果运行一个或多个 observe chunk，通过真实 P9/P10 API 写入
   O1/O2/E1/E2 hard Bank/Identity/FSM，再经 Retriever→Reader→Resampler→Composer 执行一次
   teacher-forced Qwen prefill。Semantic Projector soft 路径保留梯度，hard records 始终
   detach/clone；Reader 禁止读取 ground-truth count，Stage A 禁止 decode 和 Inner SGD counter。
4. A1 精确为 `L_answer`，A2 精确为 `L_state+L_answer`；两者都不调用 P14 TTT loss、
   Predictor 或 functional SGD。平衡采样对 O1/O2/E1/E2 使用确定 seed 过采样，
   Outer step 执行 finite 检查和 FP32 global clip=1，非有限梯度整步 skip。
5. 指标独立输出 O1 soft/hard count、O2/E1/E2 duplicate/miss、9-class operator confusion/
   unsupported、time mode/span、retrieval precision/recall、Reader exact count 和 Reader/LLM
   number disagreement。零分母固定为 `null`，不伪造为 0。
6. checkpoint 仅保存 allowlisted `trainable.safetensors`、optimizer/RNG state 和 UTF-8 manifest；
   manifest 固定 fold/seed、dataset/annotation/Architecture/Git/config/tokenizer provenance 与产物 hash，
   不保存 full Qwen、transient `W_t`、temporal cache 或 FSM runtime。
7. P16 的前置 exit gate 要求 A2 synthetic marker、完整指标、Reader 稳定、零
   Reader/LLM 数字分歧、零 TTT activity、四类 hard rollout、finite/reset/cache/FSM/checkpoint
   审计、已处理失败样例与全部 hash 同时通过；任一缺失均 fail closed。该门禁已作为 P16 前置校验
   通过，且后续阶段产物继续按 hash fail closed。
8. P15 证据只来自 synthetic/tiny case，未下载视频、数据集或 8B 权重，不支持
   训练收敛、真实精度、显式状态收益或 TTT 改善结论；这些科学证据仍留 P19–P22。

## P16 已验证边界

1. Stage B 固定为 A3：每个 episode 恰好一个 Support，inner objective 只启用 `L_pred`；Support
   payload 在任何模型调用前拒绝答案、count、occurrence_times、counting_type 和
   counting_subtype，固定 seed replay 必须一致。
2. 因果顺序固定为 Support observe/hard write→每个有效视频行至多一步 functional SGD→后续
   Query；更新后的 fast matrix 不回跑当前 Support，只对后续 Query 生效。无有效 pair、非法 loss/
   gradient 或零梯度均显式 skip 并继续审计。
3. Query outer objective 固定为 after-update `L_answer+L_state+0.1*L_pred`；
   `meta_full_second_order` 路径可回传到两块 `W0`。first-order 仅作为显式解析参考，不能静默替代
   full-second-order 训练合同。
4. static-W0 before 与 adapted after 使用独立 reset 的 runtime、Bank、lifecycle 和 fast storage；
   每个 episode 重置 fast/SGD/cache/slot/Bank/FSM/audit，并记录版本、梯度、delta 与 skip 原因。
5. P16 只通过 synthetic/tiny CPU 调度、梯度、状态和防泄漏工程门禁；未验证真实视频收敛、真实
   8B 训练或 TTT 科学收益，这些证据仍留 P19–P22。

## P17 已验证边界

1. Stage C 隔离 A4/A5：A4=`pred+0.5*identity`，A5=`A4+0.5*event`；两者配置差异只允许新增
   event term，禁止把多个变量同时混入消融。
2. 相邻 chunk 的 O2/E1/E2 overlap snapshot 固定 detach+clone、独立 storage；position/timestamp
   因果 matcher 保留 matched/mismatch/duplicate/low-confidence/invalid-source 审计，O2 还保留
   authoritative hard identity decision，E1/E2 分开报告。
3. Support schedule 固定覆盖 1/4/8 个连续 chunk，每个有效 chunk 至多一步且 next-only；invalid
   chunk 可 skip，但时间线和后续有效更新继续。多 Query 使用独立 prefill lifecycle，逐点计算
   outer objective 后求均值，较晚 Query 的标签不得影响较早 Query。
4. 8-Support backward 后 graph tensor 必须可回收，连续 episode 不得累积 graph 节点；synthetic
   A4-vs-A3、A5-vs-A4 配对 CI 和失败案例仅是工程工具，不支持科学增益结论。
5. P17 尚未把独立 missed-new identity 指标接入 runner；CPU graph 回收也不能替代 GPU 峰值显存
   曲线。因此该阶段可称 synthetic/tiny CPU 工程闭环，但不能称全量资源/指标验收完成。
6. P17 未验证真实 8B 增益、阈值或置信结论；这些仍属于 P19/P21/P22。

## P18 runtime 骨架已验证边界

1. per-video runtime manager 原子 reset fast/SGD/cache/slot/State/Identity/FSM/Reader audit，记录
   owner 与 SHA256；正常完成、异常和 abort 都无条件 release，连续视频不得共享可变状态。
2. 每个 chunk 先按 query_time 因果裁剪，再 observe/hard write，最后调用注入式
   `TTTUpdateStage`；骨架只验证一次 update attempt、fast/optimizer-only mutation 与 next-only
   版本边界。生产 stage 尚未把真实 `L_TTT` 接到 `functional_sgd_steps_from_ttt`。
   `StageARuntimeBridge` 使用真实 `StageABankWriter` 回填 hard records。
3. module-local `use_fast_state()` 必须被真实 Fast Adapter 消费，owner row、fast version 和 update
   count 必须一致；stale、重入、版本错配或越权状态修改均 fail closed。
4. 每个 Query 只执行一次 authoritative Reader→compose→prefill；decode 只推进 driver 状态，
   Bank/FSM/fast/cache checksum 保持不变。OK/EMPTY/UNSUPPORTED/INVALID、retry/new query、future
   suffix、nested denylist 与失败释放均有协议覆盖。
5. P18 只证明 synthetic/tiny CPU runtime 骨架与失败边界；`GenerationDriver` 尚未在真实
   Qwen3-VL-8B 上运行，真实 generation、BF16、FlashAttention、DeepSpeed、多 GPU、显存、吞吐和
   效果仍由 P19+ 验证。
...
2. Stage B 已按单步 Meta-TTT 验证：support 只使用无标签 inner loss，后续 query 使用有标签
   outer loss。
3. Stage C 已按 4 到 8 个连续 support chunks、每个有效 chunk 一步 SGD 和多个后续 query points
   验证，并保留 1-Support 对照。
...
8. O2/E1/E2 overlap consistency 固定为“当前 chunk prediction → detached previous
   ...
   low-confidence/invalid-source 状态属于强类型有效性合同；跨 chunk snapshot/match 编排已由 P17
   验证。
...
14. P14 的 differentiable update 固定为 `meta_full_second_order`（`create_graph=True`、不 detach），
    online 为 `online_leaf`；P16 已实现显式 first-order 解析参考，真实训练消融仍只能在 P21 比较。
