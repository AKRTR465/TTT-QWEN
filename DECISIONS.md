...
Router 和 Time Window Resolver；P5 已通过 Fast Adapter 的本地合成张量工程门禁；P6/P7 已通过
空间对象编码器与时间事件编码器的本地合成张量工程门禁。
Observation、hard state、训练和推理入口仍按后续 Part 实现。详细论证见
...
2. `config.py` 使用 immutable/extra-forbid Pydantic schema；固定维度、容量、9 个 operator、
   ...
   156.718819M。这些后续值不能倒推为 P1 当时已经验证的实现事实。
...
10. 固定拓扑精确为 48,438,272 参数；position、mask、cache、overlap 和 owner audit 都是零参数。
    P8 精确预算冻结后，当前新增模块分项和为 156,718,819。
...
7. 精确参数为 O1 2,632,710、O2 2,103,042、E1 9,584,643、E2 7,094,792，P8 合计
   21,415,187；按现有其余分项口径，当前新增模块分项和为 156,718,819。
