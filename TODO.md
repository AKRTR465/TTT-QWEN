...
- [ ] 记录 `ARCHITECTURE.md` 规范名、修订日期和文件 hash。
- [ ] 记录 Python 3.12、PyTorch 2.9.0、Transformers 4.57.1 和 CUDA 环境。
- [ ] 记录本机/服务器各自职责和平台差异。
- [ ] 运行并保存基线命令：`uv sync --frozen`、`uv run pytest -q`、`uv run ruff check .`、`uv run mypy src`。
- [ ] 记录当前模型、数据和输出路径只来自环境变量/路径配置，禁止硬编码 Windows 或 Linux 绝对路径。
- [ ] 为 0–22 章建立需求 ID，例如 `ARCH-04-FAST-001`。
- [ ] 把本 TODO 末尾追踪矩阵纳入评审，确保每个源章节至少有一个实施阶段和一个验收位置。
- [ ] 固定第一版禁止项清单，并在代码评审模板中加入检查框。
- [ ] 定义实验命名规则，必须包含规范版本、数据折、seed、模型 revision 和 TTT 开关。
- [ ] 定义每阶段产物目录：配置快照、日志、checkpoint、指标、审计 JSON、失败样例。
- [ ] 明确本机负责模块/FSM/loss/小张量测试，服务器负责 8B/视频/FlashAttention/DeepSpeed/多 GPU。
- [ ] 建立“计划设计”和“已验证实现”两列状态，禁止文档先行后被误报为已完成。

### 实施后验收项

- [ ] 能从任一源章节反查到对应 P 和验收测试。
- [ ] 基线测试、Ruff、mypy 均有原始日志。
- [ ] v5 固定项、禁止项和实验待定项三者已分开。
- [ ] 没有把旧 v3 配置值写成 v5 运行事实。
- [ ] 没有修改模型行为。

### 交付物与退出条件

- [ ] 交付规格锁、环境快照、仓库基线报告和需求追踪表。
- [ ] P0 评审通过后才允许修改配置或创建模型模块。
