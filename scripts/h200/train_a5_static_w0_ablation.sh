...
usage() {
  cat >&2 <<'EOF'
usage:
  bash scripts/h200/train_a5_static_w0_ablation.sh <a2_checkpoint> <v4_manifest>
...
STATIC_YAML="$PROJECT_ROOT/configs/h200/a5_static_w0_k8_vithalf_decoder8_4gpu.yaml"

if [[ ! -f "$STATIC_YAML" ]]; then
  echo "static-W0 A5 config not found: $STATIC_YAML" >&2
  exit 1
fi

export YAML="$STATIC_YAML"
export TTT_A5_ADAPTATION_MODE="static_w0"
...
export RUN_ID="${RUN_ID:-$(date +%y%m%d_%H%M%S)_a5_static_w0_dense_querybundle_v4_4epoch_finalonly}"
export SESSION="${SESSION:-a5_static_w0_${RUN_ID}}"
