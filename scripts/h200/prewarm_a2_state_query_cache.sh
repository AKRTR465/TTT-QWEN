...
CACHE_ROOT="${TTT_PREPROCESS_CACHE_ROOT:-$PROJECT_ROOT/.cache/preprocess/260720_ttt8_benchmark}"
CACHE_NAMESPACE="${TTT_PREPROCESS_CACHE_NAMESPACE:-544334e7d7bbf4c2f651}"
...
RUN_ID="${TTT_CACHE_RUN_ID:-$(date +%m%d_%H%M%S)_svcbench_a2_state_query_cache_train}"
RUN_ROOT="${TTT_CACHE_RUN_ROOT:-$PROJECT_ROOT/runs/$RUN_ID}"
LOCK_DIR="$CACHE_ROOT/.state_query_train_prewarm.lock"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "another State Query cache prewarm owns $LOCK_DIR" >&2
  exit 3
fi
...
inspect = json.loads((root / "cache_inspect.json").read_text())
summary = {
    "status": "complete" if failed == 0 and not any(statuses.values()) else "failed",
    "split": "train",
    "roles": ["state_query"],
