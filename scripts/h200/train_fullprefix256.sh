...
if [[ "$STAGE" == "a2" ]]; then
  ...
  DEFAULT_VISUAL_COST_INDEX="$PROJECT_ROOT/artifacts/a2_state16_answer256_ema_visual_cost_index.json"
  ...
  if [[ ! -f "$VISUAL_COST_INDEX" && "${TTT_VISUAL_COST_PREFLIGHT:-0}" != "1" ]]; then
    echo "measured schema-4 State16/Answer256 visual cost index not found: $VISUAL_COST_INDEX" >&2
    exit 1
  fi
else
