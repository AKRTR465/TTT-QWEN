...
Runs four-GPU A5 with the Bank-conditioned Associative LTTT design:
  - initialize the complete Outer model from the supplied A2 final checkpoint
  - use Meta-TTT with masked FP32 visual association MSE for Support updates
...
export RUN_ID="${RUN_ID:-$(date +%y%m%d_%H%M%S)_a5_associative_lttt_dense_querybundle_v4_4epoch_finalonly}"
