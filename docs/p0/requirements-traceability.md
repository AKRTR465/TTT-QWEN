...
| ARCH-02-FLOW-001 | `2` | 总体数据流 | P3–P18 | `tests/test_fast_ttt.py`、`tests/test_state_encoder.py`、`tests/test_end_to_end_demo.py` | 已定义 | P3–P11 的 Qwen boundary、Query、Adapter、时空编码、soft Observation、hard Bank/FSM、Identity Bank 与 Retriever 已验证；P12–P18 待实现 |
...
| ARCH-06-BUDGET-001 | `6.6` | v5 参数预算 | P1、P5–P12 | `tests/test_v5_config_contract.py` 及逐模块参数预算测试 | 已定义 | P5–P9 精确参数分项以及 P10 Identity Bank/P11 Retriever 的零参数边界已验证；当前新增模块分项和 156,715,683，P12 待实现 |
...
| ARCH-07-PAYLOAD-001 | `7.3` | typed payload/semantic | P9–P10 | `tests/test_state_bank.py` 字段/Projector 测试 | 已定义 | P9 五类 typed payload、FP32 Semantic Projector 与 generic O2 CRUD、P10 身份生命周期已验证 |
...
| ARCH-17-MODULE-001 | `17` | 推荐模块与职责 | P1、附录 A | `tests/test_v5_config_contract.py`、`tests/test_v5_module_skeletons.py`、P1 职责审计 | 已定义 | P1 职责及 P3–P11 的 `qwen_adapter.py`、`query_encoder.py`、`fast_ttt.py`、`state_encoder.py`、`observation_heads.py`、`state_bank.py`、`identity_bank.py`、`state_retriever.py` 已验证；其余模块待实现 |
