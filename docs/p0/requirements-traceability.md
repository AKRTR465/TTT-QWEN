...
| ARCH-02-FLOW-001 | `2` | 总体数据流 | P3–P18 | `tests/test_fast_ttt.py`、`tests/test_state_encoder.py`、`tests/test_end_to_end_demo.py` | 已定义 | P3–P12 的 Qwen boundary、Query、Adapter、时空编码、soft Observation、hard Bank/FSM、Identity Bank、Retriever、Resampler 与 Reader 已验证；P13–P18 待实现 |
...
| ARCH-06-BUDGET-001 | `6.6` | v5 参数预算 | P1、P5–P12 | `tests/test_v5_config_contract.py` 及逐模块参数预算测试 | 已定义 | P5–P9 与 P12 Resampler 精确参数分项、P10 Identity Bank/P11 Retriever/P12 Reader 零参数边界已验证；当前新增模块分项和 156,715,683 |
...
| ARCH-10-SUMMARY-001 | `10.2` | State Token 职责 | P12–P13 | `tests/test_state_reader.py` exact count 不变量 | 已定义 | P12 已验证 q_target/selected-record 软路径梯度与 Reader hard exact-count 相互独立；P13 注入职责待实现 |
...
| ARCH-17-MODULE-001 | `17` | 推荐模块与职责 | P1、附录 A | `tests/test_v5_config_contract.py`、`tests/test_v5_module_skeletons.py`、P1 职责审计 | 已定义 | P1 职责及 P3–P12 的 `qwen_adapter.py`、`query_encoder.py`、`fast_ttt.py`、`state_encoder.py`、`observation_heads.py`、`state_bank.py`、`identity_bank.py`、`state_retriever.py`、`state_reader.py` 已验证；其余模块待实现 |
