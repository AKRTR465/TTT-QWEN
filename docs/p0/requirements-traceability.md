...
| ARCH-02-FLOW-001 | `2` | 总体数据流 | P3–P18 | `tests/test_fast_ttt.py`、`tests/test_state_encoder.py`、`tests/test_model.py`、`tests/test_p13_tiny_integration.py`、`tests/test_functional_sgd.py` | 已定义 | P3–P14 的 Qwen boundary、Query、Adapter、时空编码、soft Observation、hard Bank/FSM、Identity Bank、Retriever、Reader、Resampler、Composer、单次 prefill、loss 与逐视频 SGD 已验证；P15–P18 待实现 |
...
| ARCH-07-GRAD-001 | `7.5` | 梯度边界 | P9、P14 | `tests/test_state_bank.py`、`tests/test_functional_sgd.py`、`tests/test_p14_gradient_audit.py` | 已冻结 | P9 hard detach/no-grad、独立 snapshot 与 Projector soft gradient，加上 P14 Fast→冻结 O1→loss 的 gradient presence/norm/delta 审计均已验证 |
...
| ARCH-13-STATE-001 | `13.1` | State Loss | P14–P15 | `tests/test_losses.py` task-specific supervision | 已定义 | P14 已验证四类 typed dense target 与 op/ret/time loss；P15 pre-matched target builder 和训练编排待实现 |
| ARCH-13-ANSWER-001 | `13.2` | Answer Loss/三指标 | P14–P15 | `tests/test_losses.py` 指标分离 | 已定义 | P14 已验证 causal shift、-100 context 与 number/answer/Reader 三指标；P15 数据接线待实现 |
...
| ARCH-17-MODULE-001 | `17` | 推荐模块与职责 | P1、附录 A | `tests/test_v5_config_contract.py`、`tests/test_v5_module_skeletons.py`、`tests/test_model.py`、`tests/test_losses.py`、`tests/test_functional_sgd.py` | 已定义 | P1 职责及 P3–P14 的 Qwen/Query/Fast/Encoder/Head/Bank/Identity/Retriever/Reader/Composer/model/losses/functional SGD 模块已验证；P15 及后续模块待实现 |
