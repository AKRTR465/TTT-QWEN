...
def test_eta_gate_init_saturating_the_chunk_budget_fails_before_startup() -> None:
    """The drift that made G4 unpassable must now be caught at config load.

    ``active_slots * eta_gate_init`` above ``eta_chunk_budget`` renormalizes every
    write from step zero, so the documented G4 rate (< 20%) reads 100% forever and
    the learned gate can only set eta's relative distribution, never its total.
    The guard is cross-section, which is why ``FastMemoryConfig`` could not catch
    it: 32 slots and the gate initializer live in different config sections.
    """

    raw = _raw()
    memory = deepcopy(raw["fast_memory"])
    assert isinstance(memory, dict)
    slots = deepcopy(raw["spatial_encoder"])
    assert isinstance(slots, dict)
    # Each value is individually legal -- 0.05 is below eta_max_per_slot and 32 is
    # the shipped slot count -- so only the product is wrong.
    memory["eta_gate_init"] = 0.05
    raw["fast_memory"] = memory
    assert float(slots["active_slots"]) * 0.05 > float(memory["eta_chunk_budget"])
    with pytest.raises((ValidationError, ValueError), match="eta_chunk_budget"):
        ProjectConfig.model_validate(raw)
