...
def test_slot_mean_diagnostic_is_off_unless_explicitly_enabled() -> None:
    """The diagnostic must never engage by accident: absent env var means off."""

    import importlib
    import os

    import ttt_svcbench_qwen.state_encoder as module

    assert os.environ.get(module.FORCE_SLOT_MEAN_ENV) is None
    assert module.slot_mean_diagnostic_enabled() is False

    slots = torch.randn(2, 4, 6)
    mask = torch.ones(2, 4, dtype=torch.bool)
    encoder_out = module._forced_slot_mean(slots, mask)
    # the helper itself always collapses; it is the call site that is gated
    for row in range(2):
        for slot in range(1, 4):
            assert torch.equal(encoder_out[row, slot], encoder_out[row, 0])
    torch.testing.assert_close(encoder_out.mean(dim=1), slots.mean(dim=1))

    partial = torch.tensor([[True, True, False, False], [True, False, False, False]])
    masked = module._forced_slot_mean(slots, partial)
    torch.testing.assert_close(masked[0, 0], slots[0, :2].mean(dim=0))
    torch.testing.assert_close(masked[0, 1], slots[0, :2].mean(dim=0))
    assert torch.equal(masked[0, 2], slots[0, 2])
    torch.testing.assert_close(masked[1, 0], slots[1, 0])

    previous = os.environ.get(module.FORCE_SLOT_MEAN_ENV)
    os.environ[module.FORCE_SLOT_MEAN_ENV] = "yes"
    try:
        with pytest.raises(ValueError, match="must be unset"):
            importlib.reload(module)
    finally:
        if previous is None:
            del os.environ[module.FORCE_SLOT_MEAN_ENV]
        else:
            os.environ[module.FORCE_SLOT_MEAN_ENV] = previous
        importlib.reload(module)
    assert module.slot_mean_diagnostic_enabled() is False
