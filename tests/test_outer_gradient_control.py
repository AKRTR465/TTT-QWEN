...
def test_plain_group_clipping_isolates_a_state_spike() -> None:
    qwen = _parameter(1.0, (0.6, 0.8))
    state = _parameter(1.0, (300.0, 400.0))
    w0 = _parameter(1.0, (0.03, 0.04))
    optimizer = _optimizer(
        (("qwen", 1.0e-5, qwen), ("state", 1.0e-4, state), ("w0", 1.0e-4, w0))
    )
    controller = OuterGradientController(
        load_config().outer_gradient_control,
        expected_groups=("qwen", "state", "w0"),
    )
    ...
    assert float(state.grad.norm()) == pytest.approx(0.1)
    by_name = {group.name: group for group in audit.groups}
    assert by_name["qwen"].clip_coefficient == 1.0
    assert by_name["state"].pre_clip_norm == pytest.approx(500.0)
    assert by_name["state"].post_clip_norm == pytest.approx(0.1)
    assert by_name["state"].clip_coefficient == pytest.approx(0.0002)
...
def test_zero_partition_groups_match_plain_reference(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ttt_svcbench_qwen.outer_gradient_control.version", lambda _name: "0.18.8"
    )
    parameters = (
    ...
    optimizer = _optimizer(
        (
            ...
            ("state", 1.0e-4, parameters[1]),
            ("w0", 1.0e-4, parameters[2]),
            ("predictor", 1.0e-4, parameters[3]),
        )
    )
    controller = OuterGradientController(
        load_config().outer_gradient_control,
        expected_groups=("qwen", "state", "w0", "predictor"),
    )
    ...
    assert dict(audit.metrics())["outer_grad/predictor/lr_x_post_norm"] == pytest.approx(
        1.0e-5
    )
...
def test_group_order_and_global_clip_contract_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    optimizer = _optimizer(
        (
            ("state", 1.0e-4, _parameter(1.0, (1.0,))),
    ...
    zero.optimizer.param_groups[1]["group_name"] = "state"
    zero.clip_grad = 1.0
