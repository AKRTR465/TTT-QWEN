...
# p_context broadcast (fast_ttt._centered_over_valid_tokens).  The contract
# *family* string above is a config literal pinned across four sites and the
# mechanism it names is unchanged (a gated probe over Bank-conditioned token
# keys), so only this revision counter moves; it is what the warmup-bundle
# provenance pins, so bundles produced under revision 3's uncentered key space
# refuse to load with a legible mismatch instead of silently mixing key
# semantics.
