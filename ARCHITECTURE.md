...
- Query outer loss 默认是 `answer + official weak state` 的 `legacy_sum`；实验性的
  `instant_equal` 以全局有效数归一四个 official-weak 项，固定等权后将辅助组限制为
  Answer 的至多 30%，且不维护 EMA/checkpoint 状态；
